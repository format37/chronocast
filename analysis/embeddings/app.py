...
def get_sentence_embeddings(df):
    ...
    logger.info('0. Number of sentences: {}'.format(len(df)))
    
    # Drop values where text is shorter than 10 characters
    df = df[df['Sentence'].str.len() > 10]
    logger.info('1. Number of sentences: {}'.format(len(df)))
...
def main():
    ...
    df = pd.read_csv('data/splitted/sentences.csv')
